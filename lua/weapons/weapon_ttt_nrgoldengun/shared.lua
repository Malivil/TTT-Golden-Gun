...
function SWEP:PrimaryAttack()
	...
	if tr.Entity.IsPlayer() then
		...
		elseif tr.Entity:IsRole(ROLE_INNOCENT) or tr.Entity:IsRole(ROLE_DETECTIVE) then
			...
			return
		elseif tr.Entity:IsRole(ROLE_JESTER) then
					self.Weapon:SendWeaponAnim( ACT_VM_PRIMARYATTACK )
			self.Weapon:EmitSound( Sound( "Weapon_Deagle.Single" ) )
			self:TakePrimaryAmmo(1)
			if SERVER then
			self.Owner:Kill()
			tr.Entity:TakeDamage( 999, self.Owner, self.Owner:GetActiveWeapon() )
			end
			return
		elseif tr.Entity:IsRole(ROLE_RESURRECTOR) then
					self.Weapon:SendWeaponAnim( ACT_VM_PRIMARYATTACK )
			self.Weapon:EmitSound( Sound( "Weapon_Deagle.Single" ) )
			self:TakePrimaryAmmo(1)
			if SERVER then
			...
			return
		elseif tr.Entity:IsRole(ROLE_SERIALKILLER) then
			self.Weapon:SendWeaponAnim( ACT_VM_PRIMARYATTACK )
			self.Weapon:EmitSound( Sound( "Weapon_Deagle.Single" ) )
			self:TakePrimaryAmmo(1)
			if SERVER then
			...
			return
		elseif tr.Entity:IsRole(ROLE_INFECTED) then
			self.Weapon:SendWeaponAnim( ACT_VM_PRIMARYATTACK )
			self.Weapon:EmitSound( Sound( "Weapon_Deagle.Single" ) )
			self:TakePrimaryAmmo(1)
			if SERVER then
			self.Owner:SetRole(ROLE_INFECTED)
			SendFullStateUpdate()
			end
			return
        end
